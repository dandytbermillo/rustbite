  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      await decrementOrderStockRequirements(tx, {
        outletId: transaction.outletId,
        orderId: created.id,
        requirements: parseStockRequirementsJson(transaction.stockRequirementsJson),
        now,
      });
      ...
      await tx.paymentTransaction.update({
        ...
        data: {
          ...
          completedAt:
            ...
            (isCounterAwaitingPayment ? null : now),
        },
      });

      await bumpOutletOrderVersion(tx, transaction.outletId);
