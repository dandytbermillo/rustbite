...
import type { AdminWorkspaceNotify } from "@/components/admin/workspace/AdminWorkspaceToastHost";
...
export type UsersManagementPanelVariant = "page" | "workspace-modal";
export type UsersManagementPanelData = {
  users: AdminUserRow[];
...
function normalizeDraftForOutlets(
  draft: UserDraft,
  outlets: AdminOutletRow[],
): UserDraft {
  const outletRoles: Record<string, AdminOutletRoleValue | ""> = {};
  for (const outlet of outlets) {
    outletRoles[outlet.id] = draft.outletRoles[outlet.id] ?? "";
  }
  return { ...draft, outletRoles };
}
...
function outletRoleSignature(
  ...
  return roles
    .map((role) => `${role.outletId}:${role.role}`)
    .sort()
    .join("|");
}

function userOutletRoleSignature(user: AdminUserRow) {
  return outletRoleSignature(user.outletRoles);
}

function draftOutletRoleSignature(draft: UserDraft) {
  return outletRoleSignature(serializeOutletRoles(draft.outletRoles));
}

function draftChanged(user: AdminUserRow, draft: UserDraft | undefined) {
  if (!draft) return false;
  return (
    draft.displayName !== user.displayName ||
    draft.siteRole !== (user.siteRole ?? "") ||
    draft.isActive !== user.isActive ||
    draftOutletRoleSignature(draft) !== userOutletRoleSignature(user)
  );
}

function createFormChanged(form: CreateForm) {
  return (
    form.email.trim() !== "" ||
    form.displayName.trim() !== "" ||
    form.password !== "" ||
    form.siteRole !== "" ||
    serializeOutletRoles(form.outletRoles).length > 0
  );
}

function mergeDraftsForRefresh({
  nextUsers,
  nextOutlets,
  currentDrafts,
  previousUsers,
}: {
  nextUsers: AdminUserRow[];
  nextOutlets: AdminOutletRow[];
  currentDrafts: Record<string, UserDraft>;
  previousUsers: AdminUserRow[];
}) {
  ...
  return Object.fromEntries(
    nextUsers.map((user) => {
      ...
      if (previousUser && currentDraft && draftChanged(previousUser, currentDraft)) {
        return [user.id, normalizeDraftForOutlets(currentDraft, nextOutlets)];
      }
      return [user.id, userToDraft(user, nextOutlets)];
    }),
  );
}
...
export default function UsersManagementPanel({
  ...
  useEffect(() => {
    ...
    setDrafts((current) =>
      mergeDraftsForRefresh({
        nextUsers: initialUsers,
        nextOutlets: initialOutlets,
        currentDrafts: current,
        previousUsers: users,
      }),
    );
  }, [initialOutlets, initialUsers]);
  ...
  const usersDirty = users.some((user) => draftChanged(user, drafts[user.id]));
  const createDirty = createFormChanged(createForm);
  const resetPasswordDirty = Object.values(resetPasswords).some(
    (password) => password.trim() !== "",
  );
  const stepUpDirty = showStepUp && stepUpCode.trim() !== "";
  const isDirty =
    isWorkspaceModal &&
    (usersDirty || createDirty || resetPasswordDirty || stepUpDirty);
  ...
  const refresh = () => {
    startRefresh(async () => {
      ...
      const nextOutlets = body.outlets ?? outlets;
      setUsers(body.users);
      setOutlets(nextOutlets);
      setDrafts((current) =>
        mergeDraftsForRefresh({
          nextUsers: body.users,
          nextOutlets,
          currentDrafts: current,
          previousUsers: users,
        }),
      );
      onUsersChange?.({ users: body.users, outlets: nextOutlets });
  ...
  }, [createOpen, pendingId, showStepUp, variant]);

  useEffect(() => {
    if (isWorkspaceModal) return;
  ...
  useEffect(() => {
    ...
    if (isWorkspaceModal) {
      notify?.({ message, tone, durationMs: error ? 7000 : 3200 });
      return;
    }
  ...
  }, [error, isWorkspaceModal, notice, notify]);
  ...
  }, [notificationHistory, notificationHistoryLoaded, variant]);
  ...
  return (
    <div
      data-testid="admin-users-management-panel"
      className={isWorkspaceModal ? "grid gap-4" : "max-w-[1380px]"}
    >
      {isWorkspaceModal ? (
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-sm font-semibold text-stone-500">
              {stats.total} user{stats.total === 1 ? "" : "s"} · {stats.mfa} MFA
            </div>
          </div>
          <button
            type="button"
            ...
            className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-xs font-black tracking-widest shadow-sm transition hover:brightness-105"
      ...
      <div className={`${isWorkspaceModal ? "" : "mb-6"} grid gap-3 xl:grid-cols-[1.25fr_1fr_1fr_1fr_1fr] md:grid-cols-3 grid-cols-2`}>
        <StatCard
          dark
          label="Total users"
          value={stats.total}
      ...
      <div className={`${isWorkspaceModal ? "" : "mb-4"} flex flex-wrap items-center gap-3`}>
      ...
      {createOpen && isWorkspaceModal && (
        <CreateUserInlinePanel
...
function InlineMessage({
  ...
  return (
    <div
      data-testid={
        tone === "error" ? "admin-users-inline-error" : "admin-users-inline-notice"
      }
    ...
    >
      ...
      <button
        ...
        className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-current opacity-60 hover:bg-white/70 hover:opacity-100"
      >
...
function CreateUserInlinePanel({
  createForm,
  outlets,
  passwordPolicy,
  canManageSiteAdminAccounts,
  ...
  return (
    <form
      data-testid="admin-users-create-inline-form"
      onSubmit={onSubmit}
    ...
    >
      ...
      <div className="grid gap-4 px-5 py-5 lg:grid-cols-3">
        ...
        />
        <PasswordInput
          inputId="users-next-create-password-inline"
      ...
      <div className="flex flex-wrap justify-end gap-3 border-t border-stone-200 px-5 py-4">
        <button
          ...
          onClick={onClose}
          disabled={pending}
          className="rounded-full border border-stone-300 bg-white px-5 py-2.5 text-xs font-black tracking-widest disabled:opacity-50"
        >
          CANCEL
        </button>
        <button
          disabled={pending}
          className="inline-flex items-center gap-2 rounded-full bg-stone-950 px-5 py-2.5 text-xs font-black tracking-widest text-white disabled:opacity-50"
        >
...
function StepUpInlinePanel({
  ...
  return (
    <div
      data-testid="admin-users-step-up-inline"
      className="rounded-2xl border border-amber-300 bg-amber-50 p-5"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="text-xs font-black tracking-widest text-amber-950">
